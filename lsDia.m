...
maxTime = 10*epoch; %in s. Because recording 10 epochs of 13 s each.
...
%             text(length(profile)/2,threshold(j),['avg dia = ' num2str(dia(j,i),3) ' µm'])
    ...
    %plot yellow stimuli. this is 1-off, 2-on, 10-off
    figure(i)
    subplot(3,1,2)
    ...
    print(figure(i),'-dpdf',['ROI' num2str(i)])
